...
void StaffordLEDLightOutput::loop() {
...
void StaffordLEDLightOutput::setup_state(light::LightState *state) {
  lightstate_ = state;
  ESP_LOGD(TAG, "'%s': Setting initial state", state->get_name().c_str());
  this->write_state(state);
}

void StaffordLEDLightOutput::write_state(light::LightState *state) {
  bool binary;
...
void StaffordLEDLightOutput::setup() {
...
void StaffordLEDLightOutput::dump_config() {
  ESP_LOGCONFIG(TAG, "Stafford Led Light:");
  LOG_PIN("  Pin: ", this->pin_);
}

void StaffordLEDLightOutput::output_state_(bool state) {
  if (this->pin_ != nullptr)
